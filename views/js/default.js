...
var hitNum = firebase.database().ref('currentHitNum');
      ...
      function(data){
        ...
        wholeDatabase.once("value").then(function(snapshot) {
        	...
        	if (database.ids[currId] == null) {
                ...
                var update = firebase.database().ref('ids/' + currId + '/completed/placeHolder');
                update.set("N/A");
	        ...
	        //var section = res[sectionNum].slice(0, 5);
            //section = ['a','b','c'];
	        section.unshift("");
	        section.unshift("");
	        section.unshift("");
...
function findSocial(data) {
    ...
    var arrSD = [];
   	...
   	firebase.database().ref('tempLog').set(expertAns);
...
function finished() {
    ...
    var currCount = database.completedNow[sectionCompleting]["count"];
	firebase.database().ref('completedNow/'+ sectionCompleting + '/count').set(currCount + 1);
	firebase.database().ref('completedNow/'+ sectionCompleting + '/' + currId).set(currId);
	firebase.database().ref('ids/' + currId + '/completed/' + sectionCompleting).set(sectionCompleting);
    ...
    loggedIn.once("value").then(function(snapshot) {
        snapshot.forEach(function(child) {
            if (child.val() == currId) {
            	firebase.database().ref('output/session5').push(outputLog);
                child.ref.remove();
            }
        });
    });
